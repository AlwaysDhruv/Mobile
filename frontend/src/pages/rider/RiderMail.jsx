...
export default function RiderMailPage() {
  ...
  async function loadContacts() {
    ...
    const riders = await get("/auth/search?role=rider");
    setContacts([...sellers, ...buyers, ...riders]);
  }
    ...
    <div className="mail-page">
      ...
      <div className="mail-content">
        ...
        <table className="mail-table">
          <thead>
            <tr>
              <th>{view === "inbox" ? "From (Email)" : "To (Email)"}</th>
              <th>Subject</th>
              <th>Message</th>
              <th>Date</th>
            </tr>
          ...
          <tbody>
            {(view === "inbox" ? received : sent).map((m) => (
              <tr key={m._id}>
                <td>
                  {view === "inbox"
                    ? m.senderEmail || m.sender?.email || "Unknown"
                    : m.recipientEmail || m.recipient?.email || "Unknown"}
                </td>
                <td>{m.subject}</td>
                <td>{m.message}</td>
